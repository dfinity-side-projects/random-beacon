...
func testComparison() {
	...
	sk := sec.SecretKey()
	fmt.Println("sk = sec.SecretKey(): ", sk.String())
	...
	//pub2 := bls.PublicKeyFromSeckey(sec)
	//fmt.Println("pub2: ", pub2.String())
	...
	asig := bls.AggregateSigs([]bls.Signature{sig, sig})
	verifyTrue(bls.VerifyAggregateSig([]bls.Pubkey{pub, pub}, []byte("hi"), asig))
}
